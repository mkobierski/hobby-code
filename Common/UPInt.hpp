#ifndef _UPINT_INCLUDED
#define _UPINT_INCLUDED
...
//			UPInt operator+(vector<int> b);
//			UPInt operator+(UPInt b);
			...
			static std::string print(UPInt& printNumber); //done
...
//			static std::string print(vector<int> printVector); //check if this is required even..
		...
		protected:
			UPInt(UPInt const & b); //placeholder?
			UPInt const & operator=(UPInt const & b); //placeholder?
	...
	UPInt const & operator+=(UPInt const &, UPInt const &);

}

#endif // _UPINT_INCLUDED
